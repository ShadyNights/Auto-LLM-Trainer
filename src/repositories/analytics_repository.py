from src.domain.dto.ui_dtos import AnalyticsDTO, BudgetStat, DestinationStat, PipelineStat, RatingStat
    ...
    def get_analytics(self) -> AnalyticsDTO:
        ...
        try:
            with self.db.get_cursor() as cur:
                ...
                row = cur.fetchone()
                if row:
                    dto.recent_conversations = row["cnt"]

                cur.execute("SELECT COUNT(*) as cnt FROM itineraries WHERE created_at > NOW() - INTERVAL '30 days'")
                row = cur.fetchone()
                if row:
                    dto.recent_generations = row["cnt"]

                # Destinations
                cur.execute("SELECT destination, trip_count, avg_rating FROM popular_destinations LIMIT 10")
                for r in cur.fetchall():
                    dto.destinations.append(
                        DestinationStat(r["destination"], r["trip_count"], float(r["avg_rating"] or 0.0))
                    )

                # Ratings
                cur.execute(
                    "SELECT rating, COUNT(*) as count FROM itineraries WHERE rating > 0 GROUP BY rating ORDER BY rating DESC"
                )
                for r in cur.fetchall():
                ...
                # Budgets
                cur.execute(
                    "SELECT budget_level, COUNT(*) as count FROM trips GROUP BY budget_level ORDER BY count DESC"
                )
                for r in cur.fetchall():
