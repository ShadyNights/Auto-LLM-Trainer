    ...
    def get_ai_metrics(self) -> AIOverviewDTO:
        ...
        try:
            with self.db.get_cursor() as cur:
                ...
                cur.execute(
                    "SELECT AVG(rating) as avg_rtg, AVG(quality_score) as avg_qs, AVG(generation_time_ms) as avg_time FROM itineraries"
                )
                row = cur.fetchone()
                if row:
