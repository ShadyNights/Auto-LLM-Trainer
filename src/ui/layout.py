...
def render_header(title: str, subtitle: str | None = None, icon: str | None = None):
    """Renders a page or section header (Maps to Headline 2 or 3)."""
    icon_html = (
        f"<span aria-hidden='true' class='l-icon material-symbols-outlined' style='font-size: 24px; vertical-align: middle;'>{icon}</span>"
        if icon
        else ""
    )
...
def render_section(title: str, subtitle: str | None = None, icon: str | None = None):
    """Renders a standardized section layout."""
    icon_html = (
        f"<span aria-hidden='true' class='l-icon material-symbols-outlined' style='font-size: 20px; vertical-align: middle;'>{icon}</span>"
        if icon
        else ""
    )
    subtitle_html = f"<p>{subtitle}</p>" if subtitle else ""
