...
def render_database_manager_view(repo: DatabaseRepository):
    ...
    with c1:
        render_metric_card("Itineraries", str(repo.get_record_count(TableType.ITINERARIES)), "map")
    with c2:
        render_metric_card("Conversations", str(repo.get_record_count(TableType.CONVERSATIONS)), "forum")
    with c3:
        render_metric_card("Events", str(repo.get_record_count(TableType.EVENTS)), "bolt")
    with c4:
        render_metric_card("Audit Logs", str(repo.get_record_count(TableType.AUDIT_LOGS)), "security")
    ...
    table_map = [
        TableType.ITINERARIES,
        TableType.CONVERSATIONS,
        TableType.EVENTS,
        TableType.DATASETS,
        TableType.MODEL_VERSIONS,
        TableType.PROMPTS_METADATA,
        TableType.CONFIG_EVALUATIONS,
        TableType.TRAINING_QUEUE,
        TableType.AUDIT_LOGS,
        TableType.SYSTEM_CONFIG,
        TableType.TRIPS,
    ]
