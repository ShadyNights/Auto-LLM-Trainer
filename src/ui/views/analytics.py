...
def render_analytics_view(repo: AnalyticsRepository):
    ...
    with tab1:
        ...
        c1, c2 = st.columns(2)
        with c1:
            render_metric_card("Generations", str(analytics.recent_generations), "bolt")
        with c2:
            render_metric_card("Conversations", str(analytics.recent_conversations), "forum")

        st.markdown("#### Pipeline Growth")
        c1, c2, c3 = st.columns(3)
        with c1:
            render_metric_card("Datasets", str(analytics.pipeline.total_datasets), "dataset")
        with c2:
            render_metric_card("Prompts", str(analytics.pipeline.total_prompts), "code")
        with c3:
            render_metric_card("Models", str(analytics.pipeline.total_models), "psychology")
    ...
    with tab2:
        ...
        if analytics.destinations:
            df = pd.DataFrame(
                [
                    {"Destination": d.destination, "Trips": d.count, "Avg Rating": d.avg_rating}
                    for d in analytics.destinations
                ]
            )
