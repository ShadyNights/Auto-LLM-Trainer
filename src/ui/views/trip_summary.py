...
def render_trip_summary_view(repo: TripRepository):
    ...
    for idx, summary in enumerate(summaries):
        ...
        with st.expander(
            f"{summary.destination} ({summary.duration} Days) - {summary.created_at.strftime('%Y-%m-%d %H:%M')}",
            expanded=(idx == 0),
        ):
            # Hero Summary
            ...
            with c1:
                render_metric_card("Complexity", f"{summary.trip_complexity} pts", "timeline")
            with c2:
                render_metric_card("Quality Score", f"{summary.quality_score or 0.0:.1f}", "analytics")
            with c3:
                render_metric_card("Gen Time", f"{summary.generation_time_ms or 0} ms", "timer")
            with c4:
                render_metric_card("Conversation", f"#{summary.conversation_id}", "forum")
