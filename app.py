...
try:
    ...
    if current_page == "Generator":
        ...
        if submitted:
            ...
            if not city_clean:
                ...
                DashboardState.set_last_trip_params(
                    {
                        "city": city_clean,
                        "days": days,
                        "budget": budget,
                ...
                req = ItineraryRequest(
                    city=city_clean, budget=budget, trip_days=days, interests=interest_list, travel_style=[travel_style]
                )
        ...
        if "itinerary" in st.session_state and not submitted:
            rating, comments, submit_fb = render_results_dashboard(
                st.session_state.itinerary, st.session_state.itinerary_id, st.session_state.corr_id
            )
    ...
    st.error(
        "🚨 **System Error:** An unexpected issue occurred while processing your request. Our team has been notified."
    )
